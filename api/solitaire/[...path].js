...
module.exports = async (req, res) => {
  const relPath = req.url.replace(/^\/api\/solitaire/, '');
  const subPath = relPath.split('?')[0].replace(/^\//, '');
  const upstreamUrl = `${API_BASE_URL}/api/solitaire${relPath}`;
  ...
  try {
    ...
    if (!response.ok) {
      console.error(
        `Upstream error ${response.status} for /api/solitaire/${subPath}:`,
        data
      );
    }

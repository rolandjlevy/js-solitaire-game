export class Block {
    ...
    this.div.addEventListener(
      ...
      (e) => {
        const clickedBlock = blocks.find(
          (bl) => bl.div.id == e.currentTarget.id
        );
  ...
  isValidTake({ target, blocks }) {
    ...
    if (target.x == orig.x && Math.abs(target.y - orig.y) % 2 == 0) {
      const midVertId =
        Number(orig.div.id) + game.rowLength * (orig.y < target.y ? 1 : -1);
  ...
  takeMarble({ target, blocks, marbleToTake }) {
    ...
    const originBlock = blocks.find(
      (item) => item.div.id == game.origin.div.id
    );
    ...
    const marbleToTakeBlock = blocks.find(
      (item) => item.div.id == marbleToTake.div.id
    );
